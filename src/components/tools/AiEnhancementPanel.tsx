...
export function AiEnhancementPanel({ taskType, toolSlug, title, input }: AiEnhancementPanelProps) {
  ...
  return <section className="ai-enhancement-card ai-only-panel" aria-labelledby={`${toolSlug}-ai-title`}>
    <div className="ai-enhancement-heading"><div><h3 id={`${toolSlug}-ai-title`}>{title}</h3><p>结果由智能生成服务提供，请人工核对事实、数字和语气。</p></div></div>
    <div className="workspace-actions ai-enhancement-actions">
      <button type="button" className="primary-button" onClick={generate} disabled={!hasInput || status === "loading"}>{status === "loading" ? <ProcessingStatus label="正在生成…" /> : <><WandSparkles size={16} />开始生成</>}</button>
      {result && <><CopyButton value={result} /><TextDownloadButton value={result} name={`${toolSlug}-result.txt`} label="下载结果" /><button type="button" className="soft-button" onClick={reset}><RotateCcw size={15} />重新生成</button></>}
    </div>
    ...
    {result && <div className="ai-enhancement-result" aria-live="polite"><div className="ai-result-heading"><strong>生成结果</strong><span>请人工核对事实、数字和语气</span></div><pre>{result}</pre></div>}
    {result && <HistoryControls toolSlug={`${toolSlug}-ai`} content={result} title={`${title}生成结果`} />}
    <p className="ai-privacy-hint">部分智能生成工具会将当前输入发送到云端模型服务处理，请勿输入密码、身份证号、银行卡号或未公开的敏感信息。</p>
  </section>;
}
