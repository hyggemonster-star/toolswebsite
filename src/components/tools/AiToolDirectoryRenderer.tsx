...
export function AiToolDirectoryRenderer({ tool }: { tool: ToolRecord }) {
  ...
  return <div className="workspace-card">
    <WorkspaceHeader title={tool.name} description="按使用场景浏览一组可继续核对的智能服务入口；本目录只做场景索引，不读取实时价格、热度或平台推荐。" />
    <div className="ai-directory-toolbar">
      <label className="search-field ai-directory-search">
        <Search size={18} />
        <span className="sr-only">搜索智能工具</span>
    ...
    <div className="filter-row" aria-label="智能工具场景筛选">
    ...
    </article>)}</div> : <div className="empty-state ai-directory-empty"><span>⌕</span><strong>没有找到匹配的智能工具</strong><p>换个关键词，或者先查看全部场景。</p></div>}
