...
import { compressAudio, isAudioFile, MAX_AUDIO_COMPRESS_SECONDS, validateAudioFile, type AudioOutput } from "@/lib/audio";
...
function AudioOutputPanel({ output }: { output: AudioOutput | null }) {
  const url = useObjectUrl(output?.blob ?? null);
  if (!output) return null;
  return <div className="audio-output-card"><div className="video-output-heading"><span>处理结果</span><small>{output.name} · {formatBytes(output.blob.size)}</small></div><audio src={url || undefined} controls preload="metadata" /><FileDownloadLink url={url} name={output.name} label="下载压缩音频" /></div>;
}
