...
export const MAX_VIDEO_COMPRESS_SECONDS = 5 * 60;
...
function compressedVideoOutputName(name: string) {
...
export async function compressVideo(file: File, videoBitsPerSecond = 1_500_000): Promise<VideoOutput> {
  validateVideoFile(file);
  const mime = getRecorderMimeType();
  if (!mime) throw new Error("当前浏览器没有可用的 WebM 视频录制格式，请使用最新版 Chrome、Edge 或 Firefox。 ");
  const url = URL.createObjectURL(file);
  ...
  try {
    ...
    capture = getVideoCaptureStream(video);
    if (!capture.getVideoTracks().length) throw new Error("浏览器没有提供可录制的视频画面。 ");
    const bitrate = Math.min(4_000_000, Math.max(400_000, Math.round(Number(videoBitsPerSecond) || 1_500_000)));
    const recorder = new MediaRecorder(capture, { mimeType: mime, videoBitsPerSecond: bitrate, audioBitsPerSecond: 96_000 });
    const chunks: Blob[] = [];
    const recording = new Promise<Blob>((resolve, reject) => {
      recorder.addEventListener("dataavailable", (event) => { if (event.data.size) chunks.push(event.data); });
      recorder.addEventListener("stop", () => resolve(new Blob(chunks, { type: mime })), { once: true });
