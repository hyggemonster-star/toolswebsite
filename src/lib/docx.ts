...
function findEndOfCentralDirectory(bytes: Uint8Array) {
  ...
  throw new Error("无法读取 DOCX 压缩包目录，文件可能损坏。 ");
}

function getZipEntry(bytes: Uint8Array, fileName: string) {
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  ...
  for (let index = 0; index < entryCount; index += 1) {
    if (view.getUint32(cursor, true) !== 0x02014b50) throw new Error("DOCX 压缩包目录不完整。 ");
    ...
    if (name === fileName) {
      if (uncompressedSize > MAX_DOCX_TEXT_CHARACTERS * 8) throw new Error("DOCX 正文解压后过大，请先精简文档。 ");
      const localView = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
      if (localView.getUint32(localOffset, true) !== 0x04034b50) throw new Error("DOCX 正文入口无效。 ");
...
async function decompressZipEntry(entry: { method: number; compressed: Uint8Array }) {
  if (entry.method === 0) return entry.compressed;
  if (entry.method !== 8 || typeof DecompressionStream === "undefined") throw new Error("当前浏览器不支持 DOCX 解压，请使用最新版 Chrome、Edge 或 Firefox。 ");
...
export async function extractDocxText(file: File) {
  ...
  const xmlBytes = await decompressZipEntry(getZipEntry(bytes, "word/document.xml"));
