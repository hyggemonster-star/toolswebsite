...
- 当前阶段：第二阶段 Stage 36（本地 AI 工具场景目录）已完成；77 个工具已有真实操作区，100 个工具均拥有独立页面与基础 SEO，下一阶段评估场景工具包与求职/学习方向的可维护扩展
...
当前公网入口：`http://101.43.29.216:39090/`。Stage 36 静态产物已切换到独立目录；首页、`/tools`、AI 工具导航/小红书 Prompt/短视频 Prompt/电商 Prompt/文本表达/长文重点/面试准备/工作周报/简历内容/PPT 大纲/评论回复/朋友圈文案/公众号标题/公众号排版、PDF 转图片/证件照换底色/图片去背景/图片增强、短视频分镜/抖音标题/口播脚本生成器、小红书标题结构/标签/排版/敏感词工具、Markdown/Prompt 内容工具、6 个场景工具包、代表工具页、`robots.txt`、`sitemap.xml` 外部复验均返回 200，PDF.js worker 资源也可公网访问。100 个详情页均已输出 canonical、Open Graph、JSON-LD、FAQ 和独立工具元数据；发布前旧站目录备份位于 `/www/backup/tools-hub-100-stage36-before-20260911`，仍可回滚到上一版本。
...
1. 进入 Stage 37：优先评估场景化工具包与求职/学习方向的真实入口，补齐“解决一件事”的任务路径；AI 外部目录只维护静态官方入口，不接入未经验证的实时价格、热度或第三方 API；HEIC 转 JPG 继续保持兼容性评估，不在浏览器不稳定时伪装上线。
