...
- 当前阶段：第一阶段本地实现与专业级 UI/UX 视觉重构完成，GitHub 与腾讯云已同步，公网入口复验通过
...
1. 根据真实使用反馈选择 PDF、图片/媒体或 AI 工具的下一批本地化路线。
2. 正式域名确认后配置 `NEXT_PUBLIC_SITE_URL`，并补做真实设备视觉验收。
3. 暂不修改 Hansik/StockAI；继续保持本项目的代码、目录、端口和部署配置独立。
...
- 将未实现的 85 个工具保留为可索引的产品路线，但不让“即将上线”在首页抢走已可用工具的注意力。
